(function () {
  ...
  (function setupNewGameBtn() {
    const newGameBtn = document.querySelector(".main-menu__new-game");
  ...
  function resetGameState(gameState) {
    ...
    Object.keys(gameState.keysDown).forEach(
      (prop) => (gameState.keysDown[prop] = false)
    );
  }
  ...
  function createEnemies(gameState) {
    const { canvas, enemies } = gameState;
  ...
  function isGameObjOnMap(
    ...
    return y + height < mapHeight && x + width < mapWidth && y > 0 && x > 0;
  }
  ...
  function render(gameState) {
    const { background, player, bullets, enemies } = gameState;
